    ...
    <script>
            ...
            usageChart = new Chart(usageCtx, {
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: { grid: { display: false }, beginAtZero: true, ticks: { stepSize: 1, precision: 0 } }
                    }
                }
            });
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('btn-export-csv').addEventListener('click', function(){
                ...
                const url = `{{ route('super.reports.export.excel') }}` + (q ? (`?q=`+encodeURIComponent(q)) : '');
            ...
            document.getElementById('btn-print-pdf').addEventListener('click', ()=>{
                const q = document.getElementById('rec-search')?.value || '';
                const url = `{{ route('super.reports.export.pdf') }}` + (q ? (`?q=`+encodeURIComponent(q)) : '');
                window.location.href = url;
            });
        });
    </script>
